...
const Settings = () => {
    ...
    return (
        <div className="bg-background-light text-text-dark font-display min-h-screen flex justify-center">
            <div className="relative flex h-full min-h-screen w-full max-w-md flex-col bg-background-light overflow-x-hidden shadow-2xl">
                {/* Top App Bar */}
                <div className="flex items-center bg-background-light p-4 pb-2 sticky top-0 z-10">
                    <button
                        onClick={() => navigate(-1)}
                        className="text-text-dark flex size-12 shrink-0 items-center justify-center cursor-pointer bg-gray-100 rounded-full hover:bg-gray-200 transition-colors"
                    >
                        <span className="material-symbols-outlined text-xl">arrow_back_ios_new</span>
                    </button>
                    <h2 className="text-text-dark text-lg font-bold leading-tight tracking-tight flex-1 text-center pr-12">Settings</h2>
                </div>

                <div className="flex flex-col flex-1 pb-24">
                    {/* Account Management Section */}
                    <h3 className="text-text-dark text-lg font-bold leading-tight tracking-[-0.015em] px-4 pb-2 pt-6">Account Management</h3>

                    {/* Contact Us ListItem */}
                    <button
                        onClick={handleContactUs}
                        className="flex items-center gap-4 bg-white mx-4 mt-2 p-4 rounded-xl border border-gray-100 shadow-sm transition-transform active:scale-95 w-[92%]"
                    >
                        <div className="flex items-center gap-4 flex-1">
                            <div className="text-primary flex items-center justify-center rounded-lg bg-pink-100 shrink-0 size-10">
                                <span className="material-symbols-outlined">chat</span>
                            </div>
                            <p className="text-text-dark text-base font-medium leading-normal flex-1 text-left truncate">Contact Us</p>
                        </div>
                    ...
                    <div className="flex items-center gap-4 bg-white mx-4 mt-4 p-4 rounded-xl border border-gray-100 shadow-sm">
                        <div className="flex items-center gap-4 flex-1">
                            <div className="text-primary flex items-center justify-center rounded-lg bg-pink-100 shrink-0 size-12">
                                <span className="material-symbols-outlined">ac_unit</span>
                            </div>
                            <div className="flex flex-col justify-center">
                                <p className="text-text-dark text-base font-medium leading-normal">Freeze Account</p>
                                <p className="text-primary text-sm font-normal leading-tight">Temporarily hide your profile</p>
                            </div>
                        </div>
                        <div className="shrink-0">
                            <label className="relative flex h-[31px] w-[51px] cursor-pointer items-center rounded-full border-none bg-gray-200 p-0.5 transition-colors duration-200 has-[:checked]:bg-primary">
                    ...
                    <h3 className="text-text-dark text-lg font-bold leading-tight tracking-[-0.015em] px-4 pb-2 pt-8">Account Actions</h3>

                    {/* Delete Account ListItem */}
                    <button
                        onClick={handleDeleteAccount}
                        className="flex items-center gap-4 bg-white mx-4 mt-2 p-4 rounded-xl border border-gray-100 shadow-sm cursor-pointer active:bg-gray-50 transition-colors w-[92%]"
                    >
                        <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="px-4 mt-auto pt-12 mb-6">
                        ...
                        <p className="text-center text-xs text-gray-400 mt-4 font-medium uppercase tracking-widest">Version 2.4.1 (Build 109)</p>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl scale-100 transition-transform">
                        <div className="flex flex-col items-center text-center gap-4">
                            ...
                            <h3 className="text-xl font-bold text-text-dark">Delete Account?</h3>
